    ...
    <div class="card p-3">
        ...
        <div class="table-responsive">
            <table ref="table" class="table">
                <thead>
                    <tr>
                        <th v-for="(column, index) in columns" @click="sort(index)" v-bind:key="column.index" v-bind:value="colomn.index":class="(sortable ? 'sortable' : '')
                            + (sortColumn === index ?
                ...
                <tbody>
                    <tr v-for="(row, index) in paginated"  v-bind:key="row.index"@click="click(row, index)">
                        <template v-for="column in columns">
                            <td :class="column.numeric ? 'numeric' : ''" v-if="!column.html" v-bind:key="column.id">
                                {{ collect(row,column.field) }}
                            </td>
                            <td :class="column.numeric ? 'numeric' : ''"  v-bind:key="column.numeric"v-html="collect(row, column.field)" v-if="column.html">
                            </td>
                        </template>
